...
(
    ...
    CONSTRAINT `organizations_personal_org_owner_required` CHECK ((`is_personal` = 0 and `personal_owner_user_id` is null) or
                                                                  (`is_personal` = 1 and `personal_owner_user_id` is not null))
);
--> statement-breakpoint
